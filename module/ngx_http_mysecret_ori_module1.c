...
static ngx_int_t ngx_http_mysecret_handler(ngx_http_request_t *r)
{
	...
	realfilename = ngx_alloc(len,r->pool->log);
//	realfilename[len] = '/';

	ngx_str_t str = ngx_string("/home/faith/module/input.info");
	ngx_snprintf(realfilename,len,"%V",&str);
	...
	ssize_t n = ngx_read_fd(file->fd,buf,len);

	if((size_t)n != len || (off_t)n == -1)
	{
		return NGX_ERROR;
	}
