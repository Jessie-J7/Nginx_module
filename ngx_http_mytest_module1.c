...
static void * ngx_http_mysecret_create_loc_conf(ngx_conf_t *cf)
{
...
static ngx_http_module_t ngx_http_mytest_module_ctx = {
	...
	NULL,
	NULL,
	ngx_http_mysecret_create_loc_conf,
	NULL
};
...
ngx_int_t cJSON_to_array(char *text,ngx_http_request_t *r)
{
	...
	else
	{
		...
		if(arrayItem!=NULL)
		{
			...
			for(i=0;i<size;i++)
			{
				...
				if(item != NULL)
				{
					ngx_memcpy(token,item->valuestring,ngx_strlen(item->valuestring));
				}

				if(0 == ngx_strcmp(token,myctx->session))
				{
					item = cJSON_GetObjectItem(object,"key");
					if(item != NULL)
					{
						ngx_memcpy(myctx->key,item->valuestring,ngx_strlen(item->valuestring));
					}
					break;
				}
			}
...
static ngx_int_t mytest_subrequest_post_handler(ngx_http_request_t *r,void *data, ngx_int_t rc)
{
	...
	cJSON_to_array((char *)cjson.data,r);
...
static ngx_int_t mytest_post_handler(ngx_http_request_t * r)
{
	...
	else
	{
		...
		if(ret == 0)
		{
			...
			ngx_snprintf(sub_prefix,mycf->uri_server.len,"%V",mycf->uri_server.data);
...
ngx_int_t cJSON_to_str(char *json_string,ngx_http_request_t *r)
{
	ngx_http_mytest_ctx_t* myctx = ngx_http_get_module_ctx(r, ngx_http_mytest_module);
	...
	else
	{
		...
		if(item!=NULL)
		{
			ngx_log_stderr(0,"cJSON_GetObjectItem: type=%d, key is %s, value is %s\n",item->type,item->string,item->valuestring);
			myctx->ser_data.len = ngx_strlen(item->valuestring);
			memcpy(myctx->ser_data.data,item->valuestring,myctx->ser_data.len);
		}
		cJSON_Delete(root);
	}
	return 0;
...
static ngx_int_t ngx_http_mytest_handler(ngx_http_request_t *r)
{
	...
	u_char filename[] = ngx_alloc(128,r->pool->log);
	...
	else
	{
		...
		if(ret == 0)
		{
			...
			psr_key->data = myctx;
			ngx_str_t sub_prefix;
			ngx_snprintf(sub_prefix,mycf->uri_token.len,"%V",mycf->uri_token.data);
			ngx_str_t sub_location;
			sub_location.len = sub_prefix.len;
			sub_location.data = ngx_palloc(r->pool, sub_location.len);
			ngx_snprintf(sub_location.data, sub_location.len,
