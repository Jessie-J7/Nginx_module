...
static ngx_int_t ngx_http_mytest_handler(ngx_http_request_t *r)
{
	...
	u_char *session = ngx_alloc(33,r->pool->log);
	ngx_memzero(session,33);
	ngx_str_t str1 = ngx_string("session");
	query_string(r,session,str1);         //获取参数中的session
	u_char *data = ngx_alloc(1024,r->pool->log);
	ngx_memzero(data,1024);
	ngx_str_t str2 = ngx_string("data");
	query_string(r,data,str2);           //获取data
	u_char *requesttime = ngx_alloc(16,r->pool->log);
	ngx_memzero(requesttime,16);
	ngx_str_t str3 = ngx_string("requesttime");
	query_string(r,requesttime,str3);	 //获取requesttime

	u_char key[33]={0};
	u_char filename[] = {"/home/faith/ngx_test"};
	read_file(r,filename,session,key);  //根据session，在文件中查找key

	ngx_str_t pencode;   //编码数据
	ngx_str_t pdecode;   //解码数据
	u_char *c;
	ngx_int_t clen = 0;
	c = ngx_alloc(1024,r->pool->log);
	ngx_memzero(c,1024);

	if(ngx_strlen(key) == 0) //进入token
	{
		ngx_log_stderr(0,"no session");
