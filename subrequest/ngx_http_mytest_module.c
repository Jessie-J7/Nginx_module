...
{
	ngx_log_stderr(0," ziqingqiui_post");
...
//开始解析http包体
    ...
    {
		ngx_str_set(&myctx->stock[0],"body");
		ngx_str_set(&myctx->stock[1],"hello");
    }

	ngx_log_stderr(0,"stock[0]: %V",&myctx->stock[0]);
	ngx_log_stderr(0,"stock[1]: %V",&myctx->stock[1]);
...
//游服务器的响应全部保存在子请求的sr->upstream->buffer内存缓冲区中
	...
	ngx_log_stderr(0,"after ziqingqiu");
    return NGX_DONE;
}

static ngx_int_t mytest_subrequest_post_key_handler(ngx_http_request_t *r,
                                                void *data, ngx_int_t rc)
{
	ngx_log_stderr(0," ziqingqiu");
    //当前请求r是子请求，它的parent成员就指向父请求
    ngx_http_request_t          *pr = r->parent;
    ngx_http_mytest_ctx_t* myctx = ngx_http_get_module_ctx(pr, ngx_http_mytest_module);
...
//开始解析http包体
    if (r->headers_out.status == NGX_HTTP_OK)
    {
		ngx_str_set(&myctx->stock[2],"result");
		ngx_str_set(&myctx->stock[3],"123");
    }
	ngx_log_stderr(0,"stock[2]: %V",&myctx->stock[2]);
	ngx_log_stderr(0,"stock[3]: %V",&myctx->stock[3]);
...
//stock[…],Today current price: …, volumn: …
   // ngx_str_t output_format = ngx_string("stock[%V],Today current price: %V, volumn: %V");
    ngx_str_t output_format = ngx_string("%V:%V");

    //计算待发送包体的长度
   // int bodylen = output_format.len + myctx->stock[0].len
     //             + myctx->stock[1].len + myctx->stock[4].len - 6;
    int bodylen = output_format.len + myctx->stock[2].len + myctx->stock[3].len - 4;
